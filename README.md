...
|---|---|---|---|---|
| `Regex` | stdlib-`Regex`-shaped matching | `NSRegularExpression` | `java.util.regex` (via `CHostBridge`) | JS `RegExp` |
...
let re = try regex(#"(\d{4})-(\d{2})"#)     // throwing; `rx("...")` traps, for constants
...
The type is named `Regex`, which collides with the standard library's `Regex`
at a `import Regex` call site. Use `regex(_:)` / `rx(_:)` and the `String`
matching methods (`text.firstMatch(of:)`, `text.matches(of:)`, ...) to avoid
naming the type, or qualify it as `Regex.Regex`. It does not conform to
