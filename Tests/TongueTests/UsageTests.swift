...
struct TongueUsage {
    ...
    @Test func aTurnstileMadeOnAPostedDayEmitsOnTheNextDaysFirstDetection() async throws {
        ...
        let turnstile = UsageTurnstile(client: UsageClient(ClientDeps(
            deviceId: "tongue-day",
            platform: "test",
            now: { sink.clock },
            loadState: { sink.state },
            saveState: { sink.state = $0 },
            send: { body, _ in sink.sent.append(body) }
        )), telemetry: TelemetryDebug(sends: InflightSends()))
        await turnstile.record()
