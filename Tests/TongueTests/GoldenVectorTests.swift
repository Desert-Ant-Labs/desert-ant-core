...
struct GoldenVectorTests {
    ...
    // a silently missing contract file is exactly the drift this suite exists
    // to catch.
    ...
    /// Replays detection_vectors.json: the head's output. Without these, the three
    /// ports could disagree on `language` while all three suites stay green.
    ///
