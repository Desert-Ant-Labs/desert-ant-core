...
///
/// Gist is the most expensive model here to load: building the 67 MB embedding
/// table costs ~2 s, while a classify costs ~1 ms. So the suite's runtime is
/// entirely a function of how many times it loads, and nothing else.
///
/// `Gist()` resolves through its own `LoadedModel`, which does not share
/// `ModelFixture`'s memo, so a suite that used both loaded twice. Building the
/// tagger from the fixture's already-resolved files collapses that to one,
/// shared with `PipelineTests`, which needs the same files for its oracles.
///
/// The public `Gist()` construction path is deliberately not exercised here:
/// the resolve/download machinery is core's and is covered by the Emo and
/// Redact `HubDownloadTests`, so paying for it again here buys nothing.
enum GistFixture {
    struct Loaded {
