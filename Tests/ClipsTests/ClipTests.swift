...
struct ClipTokenizerTests {
    ...
    // The fixture is a SwiftPM resource and `Bundle.module` has no WASI backing,
    // so the parity run is off-wasm; the container and truncation contracts above
    // are exercised on every platform.
#if !os(WASI)
    /// Swift ids == the Python `transformers` reference ids, over a corpus that
    /// reaches the colliding pieces: Burmese, Arabic, Hebrew, Kannada, Vietnamese,
    /// CJK, Indic, Cyrillic, Greek, and the Latin the model spends its day on.
    ///
    /// `clip_vocab_subset.bin` is xlm-roberta-base's vocab reduced to the 2,568
    /// pieces this corpus can reach, renumbered densely, with the reference ids
    /// renumbered to match. The reduction is sound rather than a sample: the
    /// decoder only ever looks up substrings of the normalized text, so a piece no
    /// text contains cannot change a result, and the pieces that carry the
    /// property under test - all 29 canonically-equivalent pairs, in their
    /// original relative order - are kept whole. It also keeps the four specials
    /// at their xlm-roberta ids and the globally lowest-scoring piece, because
    /// `unkPenalty` is `min(score) − 10` and must not move. Verified against the
    /// published 4 MB `clip_tokenizer.bin`: identical rows pass and fail on both.
    ///
    /// Committing the real vocab instead would have quadrupled a 1.6 MB repo that
    /// every SwiftPM consumer clones.
    @Test func referenceParityAcrossScripts() throws {
        struct Row: Decodable { let text: String, ids: [Int32] }
        let vocab = try #require(Bundle.module.url(forResource: "clip_vocab_subset", withExtension: "bin"))
        let reference = try #require(Bundle.module.url(forResource: "tokenizer_parity", withExtension: "json"))
        let tokenizer = try #require(Tokenizer(bytes: [UInt8](try Data(contentsOf: vocab))))
        let rows = try JSONDecoder().decode([Row].self, from: try Data(contentsOf: reference))
        #expect(rows.count > 80)

        var failures: [String] = []
        for row in rows {
            let got = tokenizer.encode(row.text, maxLength: 128)
            if got != row.ids { failures.append("\(row.text)\n  want \(row.ids)\n  got  \(got)") }
        }
        // A `Comment` takes a string literal or interpolation, not a built String.
        let sample = failures.prefix(3).joined(separator: "\n")
        #expect(failures.isEmpty, "\(failures.count) of \(rows.count) rows diverge\n\(sample)")
    }
#endif
}
