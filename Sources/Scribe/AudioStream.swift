...
struct FileAudioStream: AudioStream {
    ...
    init(url: URL, sampleRate: Double, chunkSeconds: Double = 10) throws {
        do {
            ...
            throw ScribeError.invalidAudio("\(url.path): \(error)")
        }
        ...
        else { throw ScribeError.invalidAudio("\(url.path) decoded to no audio") }
        outputFormat = output
        converter = made
    ...
    mutating func read(_ count: Int, into buffer: inout [Float]) throws -> Int {
        ...
        while produced < count && !finished {
            guard let input = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: chunkFrames) else {
                throw ScribeError.invalidAudio("cannot allocate a read buffer")
            }
            ...
            do {
                ...
                throw ScribeError.invalidAudio("read failed: \(error)")
            }
            ...
            guard let output = AVAudioPCMBuffer(pcmFormat: outputFormat,
                ...
                throw ScribeError.invalidAudio("cannot allocate a convert buffer")
            }
            ...
            if let failure { throw ScribeError.invalidAudio("convert failed: \(failure)") }
    ...
    private mutating func drain(into buffer: inout [Float]) throws -> Int {
        ...
        if let failure { throw ScribeError.invalidAudio("drain failed: \(failure)") }
