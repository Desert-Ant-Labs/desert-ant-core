...
public enum ScribeError: Error, CustomStringConvertible, Sendable {
    ...
    public var description: String {
        switch self {
        case .unsupportedPlatform:
            return "Scribe requires Core ML and runs on Apple platforms only"
...
///
/// ```swift
/// let scribe = try await Scribe()
/// let result = try await scribe.transcribe(url) { print($0.fractionCompleted) }
...
public actor Scribe {
    ...
    public static func isDownloaded(directory: String? = nil, cacheRoot: String? = nil) -> Bool {
        ScribeModel.isAvailable(directory: directory, cacheRoot: cacheRoot)
    }
    ...
    public static func download(
        ...
        try await ScribeModel.resolve(directory: directory, cacheRoot: cacheRoot,
    ...
    public init(
        ...
        guard ScribeModel.supports(.current) else { throw ScribeError.unsupportedPlatform }
        let stored = try await ScribeModel.resolve(directory: directory, cacheRoot: cacheRoot,
                                                     progress: progress)
