// Gist's exported entry points. Everything behind them is model-agnostic and
...
//
// The names are model-scoped (`gist_create`, `Java_ai_desertant_gist_...`) rather
...
#if !os(Android)
@_cdecl("gist_create")
public func gist_create(_ modelId: UnsafePointer<CChar>?, _ cacheRoot: UnsafePointer<CChar>?,
                         _ directory: UnsafePointer<CChar>?) -> UnsafeMutableRawPointer? {
    nativeCreate(binding: GistBinding.self, modelId: modelId,
...
public func androidCreateGist(_ env: UnsafeMutablePointer<JNIEnv?>, _ cls: jclass?,
                                 _ modelId: jbyteArray?, _ cacheRoot: jbyteArray?,
                                 _ directory: jbyteArray?) -> jlong {
    androidCreate(GistBinding.self, env: env, cls: cls, modelId: modelId,
                  cacheRoot: cacheRoot, directory: directory)
}

@_cdecl("Java_ai_desertant_gist_GistNative_destroy")
public func androidDestroyGist(_ env: UnsafeMutablePointer<JNIEnv?>, _ cls: jclass?,
                                  _ handle: jlong) {
    nativeDestroy(androidPointer(handle))
}

@_cdecl("Java_ai_desertant_gist_GistNative_isDownloaded")
public func androidIsDownloadedGist(_ env: UnsafeMutablePointer<JNIEnv?>, _ cls: jclass?,
                                       _ handle: jlong) -> jint {
    androidIsDownloaded(env, cls, handle)
}

@_cdecl("Java_ai_desertant_gist_GistNative_download")
public func androidDownloadGist(_ env: UnsafeMutablePointer<JNIEnv?>, _ cls: jclass?,
                                   _ handle: jlong) -> jint {
    androidDownload(env, cls, handle)
}

@_cdecl("Java_ai_desertant_gist_GistNative_run")
public func androidRunGist(_ env: UnsafeMutablePointer<JNIEnv?>, _ cls: jclass?,
