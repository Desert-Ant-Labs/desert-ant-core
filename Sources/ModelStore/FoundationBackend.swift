...
public extension ModelStore {
    ...
    /// directory. This used to discard `cacheRoot` unconditionally, so a caller
    /// that passed one (every Node host does) silently wrote somewhere else -
    /// which matters wherever the platform default is not writable, e.g. Lambda.
