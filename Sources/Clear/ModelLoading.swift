...
public struct ModelAssets: Sendable {
    ...
    static func clear(files: StoredModel, variant: ModelVariant, revision: String? = nil,
        ...
        // that is the one the placement rule is allowed to replace. Enhancement
        // is convolutional and the engine is good at it, so the GPU only takes
        // it where it is wide enough - see `Placement.neuralEngineFriendly`.
        let placement = computeUnits == .all ? Placement.neuralEngineFriendly : computeUnits
        var sessions: [any InferenceSession] = []
        for _ in 0..<max(1, concurrency) {
            sessions.append(try await files.inferenceSession(
                model: artifact, computeUnits: placement, sdk: ClearModel.sdkInfo))
        }
