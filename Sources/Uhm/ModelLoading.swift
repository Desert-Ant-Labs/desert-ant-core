...
public struct ModelAssets: Sendable {
    ...
    static func uhm(files: StoredModel, quality: Uhm.Quality,
        ...
        // that is the one the placement rule is allowed to replace. The
        // detector is a graph the Neural Engine handles badly - see
        // `Placement.gpuFriendly`.
        let placement = computeUnits == .all ? Placement.gpuFriendly : computeUnits
        return ModelAssets(
            session: try await files.inferenceSession(
                model: quality.artifact(for: .current), computeUnits: placement,
