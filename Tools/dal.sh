...
# The ONNX Runtime version vendored below. The headers checked into
# Sources/COnnxRuntime/include/onnxruntime came out of this same release, so
# moving one without the other is how the shim starts compiling against an API
# table the DLL does not have.
