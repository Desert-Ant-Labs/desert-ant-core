...
// loudly and helpfully when someone calls Moderator.load() in plain Node. LiteRT.js
...
// process (no DOM), call Moderator.load, and assert it rejects with guidance pointing
// at `@desert-ant-labs/moderator/native`. This is the Node-observable contract; the
...
const child = `
const { Moderator } = await import(${JSON.stringify(browserUrl)});
try {
...
test("default entry redirects to /native when Moderator.load() runs in Node", () => {
  ...
  assert.ok(/@desert-ant-labs\/moderator\/native/.test(out),
