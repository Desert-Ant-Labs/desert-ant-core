...
// loudly and helpfully when someone calls Schemer.load() in plain Node. LiteRT.js
...
// process (no DOM), call Schemer.load, and assert it rejects with guidance pointing
// at `@desert-ant-labs/schemer/native`. This is the Node-observable contract; the
...
const child = `
const { Schemer } = await import(${JSON.stringify(browserUrl)});
try {
  await Schemer.load({});
...
test("default entry redirects to /native when Schemer.load() runs in Node", () => {
  ...
  assert.ok(/@desert-ant-labs\/schemer\/native/.test(out),
