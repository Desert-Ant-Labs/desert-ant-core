...
`samples` is mono float32 PCM. Any finite, positive sample rate is accepted and resampled internally. Each word's `start` and `end` must be a finite number of seconds from -1 to 10,000,000; anything else (NaN, Infinity, a missing time, a huge or clearly negative one) rejects with a `RangeError` naming the word, whatever the language. Times past the end of the audio are accepted, but there is no audio there to refine against, so do not rely on what comes back for them. Every key you put on a word comes back untouched, so a word that carried a confidence or a speaker id still carries it; only `start` and `end` are replaced, and `refined` is added.
